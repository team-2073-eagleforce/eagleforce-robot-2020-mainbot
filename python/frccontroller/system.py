...
class System:
    ...
    def update_plant(self):
        ...
        if self.f:
            ...
            if random() > .5:
                val *= -1
            self.x = self.sysd.A @ self.x + self.sysd.B @ self.u #+ (random()*val)
    ...
    def export_cpp_coeffs(
        ...
        period_variant -- True to use PeriodVariantLoop, False to use
                          StateSpaceLoop
   mo     """
        from . import system_writer1

        system_writer = system_writer1.SystemWriter(
    ...
    def export_java_coeffs(self, class_name, period_variant=False):
        ...
        period_variant -- True to use PeriodVariantLoop, False to use
                          StateSpaceLoop
        """
        system_writer = fct.system_writer1.SystemWriter(
