...
def shooter(motor, num_motors, J, G):
    """Returns the state-space model for the 2020 robot shooter. Aka augmented flywheel
    ...
    A = np.array([[-G ** 2 * motor.Kt / (motor.Kv * motor.R * J), G * motor.Kt / (motor.R * J)],
                  [0, 0]])
    B = np.array([[G * motor.Kt / (motor.R * J)], [0]])
    C = np.array([[1, 0]])
    D = np.array([[0]])
...
def single_jointed_arm(motor, num_motors, J, G):
    ...
    A = np.array([[0, 1],
                  [0, -G ** 2 * motor.Kt / (motor.Kv * motor.R * J)]])
    B = np.array([[0],
                  [G * motor.Kt / (motor.R * J)]])
    C = np.array([[1, 0]])
    D = np.array([[0]])
