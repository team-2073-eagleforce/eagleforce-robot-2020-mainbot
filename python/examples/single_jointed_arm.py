...
class SingleJointedArm(fct.System):
    def __init__(self, dt):
        ...
        fct.System.__init__(
            self,
            np.array([[-12.0]]), # min output in volts
            np.array([[12.0]]), # max output in volts
            dt,
            np.zeros((2, 1)), # state matrix filled with zeros (position (theta) and velocity (omega))
            np.zeros((1, 1)), # input matrix (voltage only)
        )
